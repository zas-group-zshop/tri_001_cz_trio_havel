...
function addHeightFilter(c) {
    var b = 0;
    $(c.find(".vl-box-body ul li").get()).each(function (a) {
        if ($(this).find("input").attr("checked")) {
            b++;
            var d = $(this);
            d.insertBefore(d.siblings(":eq(0)"))
        } else {
            if (a > 4) {
                $(this).hide()
            }
    ...
    if (c.find(".vl-box-body ul li").length > 4 && c.find(".vl-box-body ul li").length != b) {
        c.find(".down-showAll").remove();
        c.append('<span class="btn btn-basic btn-sm down-showAll">Další</span>')
    }
}
...
$(document).ready(function () {
    ...
    $(".js-mobile-btn").click(function (a) {
        a.preventDefault();
        $(this).toggleClass("btn-basic");
        $("." + $(this).data("target")).toggleClass("hidden-xs")
    });
    $(".left-menu > ul > li.has-submenu > span").click(function (b) {
        b.preventDefault();
        var a = $(this).parent(".has-submenu");
        if (a.hasClass("active")) {
            a.removeClass("active")
        } else {
            a.toggleClass("submenu-is-opened")
        }
    });
    ...
    $(".js-btn-cart-advanced").click(function (a) {
        a.preventDefault();
        $(".cart-advanced-buttons").toggle()
    });
    $(document).ready(function () {
        $("#log-show-body").click(function (a) {
            a.preventDefault();
